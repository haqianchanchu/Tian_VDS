...
def honest_leader(chain, q_output, u, pre_sl):
	...
	if sl %R > CP_K or sl/R < 1:
		...
		(block, block_red) = algorithm.EditBlock(chain, j, u_leader['pk'], u_leader['sk'], u_leader['pk'], sl, u_changer['sk'])
	chain.append(block)
	for q in q_output:
		if not q[0].empty():
			q[0].get()
...
def corrupt_leader(chain_table, q_output, u, pre_sl):
	...
	for chain in chain_table:
		if len(chain) < length and length-len(chain) <= CP_K:
	...
	if len(append_chain) == 1:
		chain1 = algorithm.distinguish(append_chain[0], len(chain_table))
		del append_chain[0]
	...
	for chain in append_chain:
		...
		if sl %R > CP_K or sl/R < 1: 
			...
			u_leader = Beacon.select(sl)
			u_changer = Beacon.select(j)
			(block, block_red) = algorithm.EditBlockCorrupt(chain, j, u_leader['pk'], u_leader['sk'], u_leader['pk'], Beacon.slot_num(), u_changer['sk'])
		...
		for q in q_input:
			...
			if not q[0].empty() :
				...
				if algorithm.VerBlock(chain[-1], block_new,sl_leader) == 'right':
					...
					print("errrrrrrrrrrrrrrrrrrrrrrrrror"+str( algorithm.VerBlock(chain[-1], block_new,sl_leader)))
...
def corrupt(chain_b ,pk_list, u, count, q_input, q_output, max_slot):
	...
	while True:
		for chain in chain_table:
			if structure.Block.get(chain[-1]).sl >= max_slot+3:
				check = 1
				break
		if Beacon.slot_num() > max_slot:
			break
		if check == 1:
			break
		sl = Beacon.slot_num()
